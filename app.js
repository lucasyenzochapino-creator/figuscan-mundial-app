  ...
  return `<main class="screen">
    ...
    <section class="section">
      <form class="form" onsubmit="submitManual(event)">
        ...
        <div class="field"><label>Selección / país</label><div class="country-input-wrap"><input class="input" id="countryInput" placeholder="Escribí Argentina, Cabo Verde, Uzbekistán..." value="${editing ? escapeHtml(countryInputValue(editing.country, editing.countryName)) : ''}" autocomplete="off" autocorrect="off" spellcheck="false" autocapitalize="words" oninput="updateCountryBadge('countryInput')"><span class="country-flag-badge" id="countryInput_flag"></span></div><small class="help">Escribí el país. Si no existe, la app lo agrega. No usamos lista del navegador para que no salte el teclado.</small>${countryHintChips('countryInput', editing?.country || 'general')}</div>
...
function updateCountryBadge(inputId){
  ...
  if(!value){ badge.classList.remove('show'); badge.innerHTML = ''; return; }
  const parsed = countryFromInput(value);
  if(parsed && parsed.country && parsed.country !== 'general'){
    const c = countryById(parsed.country);
    if(c){
      badge.innerHTML = `<span class="flag">${c.flag}</span>`;
...
function detectedBox(){
  ...
  return `<div class="detected scan-success scan-card-save lux-save-panel"><div class="particles"><i></i><i></i><i></i><i></i><i></i></div>
    ...
    <section class="scan-data-panel">
      ...
      <div class="field"><label>País / selección</label><div class="country-input-wrap"><input class="input" id="detectedCountry" value="${escapeHtml(countryInputValue(preCountryId, preCountryName))}" placeholder="Ej: Portugal" autocomplete="off" autocorrect="off" spellcheck="false" autocapitalize="words" oninput="updateCountryBadge('detectedCountry')"><span class="country-flag-badge" id="detectedCountry_flag"></span></div>${countryHintChips('detectedCountry', preCountryId)}</div>
    </section>
...
31 32 33"></textarea>
    <div class="field batch-country-field"><label>País / selección para todas</label><div class="country-input-wrap"><input class="input" id="batchCountryInput" value="${escapeHtml(preCountry)}" placeholder="Ej: Argentina, Cabo Verde, Uzbekistán" autocomplete="off" autocorrect="off" spellcheck="false" autocapitalize="words" oninput="updateCountryBadge('batchCountryInput')"><span class="country-flag-badge" id="batchCountryInput_flag"></span></div>${countryHintChips('batchCountryInput', state.lastCountryId || 'general')}</div>
